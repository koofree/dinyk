...
export function useProvideLiquidity() {
  ...
  const approveUSDT = useCallback(
    async (poolAddress: string, amount: bigint) => {
      if (!usdt || !signer)
        throw new Error("No USDT contract or signer available");
  ...
  const calculateYield = useCallback(
    (
      ...
      const daysToMaturity = Math.max(
        1,
        (maturityTimestamp - now) / (24 * 60 * 60),
      );
      ...
      const potentialEarnings =
        (collateralAmount * BigInt(premiumRateBps)) / 10000n;
    ...
    async (poolAddress: string) => {
      ...
      try {
        ...
        return {
          ...
          availableCapacity:
            poolAccounting.totalAssets - poolAccounting.lockedAssets,
        };
      } catch (err) {
  ...
  const provideLiquidity = useCallback(
    async ({ tranche, round, amount }: ProvideLiquidityParams) => {
      ...
      try {
        // Check round state
        if (
          round.stateName !== "OPEN" &&
          round.stateName !== "ANNOUNCED" &&
          round.stateName !== "ACTIVE"
        ) {
          throw new Error(
            `Round is not open for liquidity. Current state: ${round.stateName}`,
          );
        }
        ...
        const sellerPosition = await pool.getSellerPosition(
          round.roundId,
          account,
        );
        const poolAccounting = await pool.getPoolAccounting();
        ...
        const shareValue =
          (shareBalanceAfter * poolAccounting.navPerShare) /
          ethers.parseEther("1");
        ...
        setError(
          err instanceof Error ? err : new Error("Failed to provide liquidity"),
        );
        throw err;
      } finally {
        setLoading(false);
