...
export function useUserPortfolio() {
  ...
  const [insurancePositions, setInsurancePositions] = useState<
  ...
  const fetchInsurancePositions = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `Found ${tokenIds.size} insurance tokens:`,
        Array.from(tokenIds),
      );
      ...
      for (const tokenId of tokenIds) {
        try {
          ...
          if (trancheId === 0 || roundId === 0) {
            console.log(
              `Invalid token info for token ${tokenId}: trancheId=${trancheId}, roundId=${roundId}`,
            );
            continue;
          }
          ...
          for (const trancheIdBN of trancheIds) {
            ...
            try {
              ...
              try {
                ...
                console.log(
                  `Found ${roundIds.length} rounds for tranche ${trancheId}:`,
                  roundIds.map((id) => Number(id)),
                );
              } catch (e) {
              ...
              for (const roundIdBN of roundIds) {
                ...
                console.log(
                  `Processing round ${roundId} for tranche ${trancheId}`,
                );
                ...
                try {
                  ...
                  let sellerPosition;
                  try {
                    sellerPosition = await pool.getSellerPosition(
                      roundId,
                      account,
                    );
                    console.log(
                      `Got seller position for round ${roundId}, account ${account}`,
                    );
                  } catch (err) {
                    console.log(
                      `Error getting seller position for round ${roundId}, account ${account}:`,
                      err,
                    );
                    // Try alternative: check if there's a different method or if parameters are reversed
                    try {
                      sellerPosition = await pool.sellerPositions(
                        roundId,
                        account,
                      );
                      console.log(
                        `Got seller position via sellerPositions mapping`,
                      );
                    } catch (err2) {
                  ...
                  if (collateralAmount > 0n || sharesAmount > 0n) {
                    ...
                    const currentValue =
                      (sharesAmount * navPerShare) / ethers.parseUnits("1", 18);
                    ...
                    // Determine status
                    let roundStatus: UserLiquidityPosition["roundStatus"] =
                      "active";
                  ...
                  console.error(
                    `Error processing round ${roundId}:`,
                    roundError,
                  );
                }
              }
              ...
              console.error(
                `Error fetching tranche ${trancheId} for product ${productId}:`,
                error,
              );
            }
          }
  ...
  const portfolioSummary = {
    ...
    totalEarnings: liquidityPositions.reduce(
      (sum, pos) =>
        sum +
        parseFloat(pos.earnedPremium || "0") +
        parseFloat(pos.stakingRewards || "0"),
      0,
    ),
